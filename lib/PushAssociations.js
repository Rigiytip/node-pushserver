...
var add = function (user, deviceType, token, lang) {
    var pushItem = new PushAssociation({user: user, type: deviceType, token: token, lang: lang});
    pushItem.save();
};
...
var outputFilterWrapper = function (callback) {
    return function (err, pushItems) {
        ...
        var items = _.map(pushItems, function (pushItem) {
            return _.pick(pushItem, ['user', 'type', 'token', 'lang'])
        });
